...
async def run() -> int:
    ...
    for input_ in target.inputs:
        print(f'inputs: "{input_.name}" ({input_}) has {len(input_.inputs)} input(s): {input_._inputs}')
        target_inputs |= set(input_.inputs)
    print("---")
    ...
    while not result:
        ...
        while pulse_queue:
            ...
            if destination in target_inputs and destination.pulse_type == HIGH:
                print(f"[{destination.name}] pulse is {destination.pulse_type.__name__} after {num_pushes} iterations")
                if all(math.gcd(num_pushes, v) == 1 for v in target_values):
                    target_values.add(num_pushes)
                print(f"[{destination.name}] {len(target_values)} of {len(target_inputs)} values: {target_values}")
                print(f"[{destination.name}] current least common multiple: {math.lcm(*target_values)}")
                print("---")

                if len(target_values) == len(target_inputs):
                    result = math.lcm(*target_values)
                    break
