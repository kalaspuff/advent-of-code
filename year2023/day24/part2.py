# note to self: need to clean this up A LOT. solved initially with z3-solver. updating to use sympy if possible
...
def calculate_rock_throwing_position(hailstones: list[Hailstone]):
    (x, y, z, vx, vy, vz), times = symbols(("x, y, z, vx, vy, vz", f"t:{len(hailstones)}"))
    ...
    # times = [Int(f"T{i}") for i in range(num_lines_to_solve)]
    #
    # solver = Solver()
    # for i in range(num_lines_to_solve):
    #     (x_i, y_i, z_i), (dx_i, dy_i, dz_i) = lines[i]
...
async def run():
    ...
    return calculate_rock_throwing_position(hailstones[:3])
